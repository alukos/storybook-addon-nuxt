...
async function useNuxtViteConfig() {
    ...
    if (nuxt.options.builder !== '@nuxt/vite-builder') {
        throw new Error(`Storybook addon Nuxt only supports Vite bundler, but Nuxt builder is currently set to '${nuxt.options.builder}'.`);
    }
    const runtimeDir = fileURLToPath(new URL('../runtime', import.meta.url));
    nuxt.options.build.templates.push({ src: join(runtimeDir, 'composables.mjs'), filename: 'storybook/composables.mjs' }, { src: join(runtimeDir, 'components.mjs'), filename: 'storybook/components.mjs' });
